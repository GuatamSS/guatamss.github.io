 ...
 */
{
	...
	class GridItem {
		...
		isNavCtrl() {
			return this.DOM.el.classList.contains('grid__item--nav3');
		}
	};
	...
	class Grid {
		constructor(el, options) {
			...
			this.DOM = {};
			this.DOM.el = el;
			this.DOM.items = Array.from(this.DOM.el.querySelectorAll('div.grid__item3'));
			this.DOM.name = this.DOM.el.querySelector('.grid__item--name3');
			this.DOM.title = this.DOM.el.querySelector('.grid__item--title3');
			this.DOM.text = this.DOM.el.querySelector('.grid__item--text3');
		...
		show(filled = false, animation = true) {
			return new Promise((resolve, reject) => {
				this.DOM.el.classList.add('grid--animating3');
				this.hideItems();

				this.DOM.el.classList.add('grid--current3');
				const promises = [];
				...
				Promise.all(promises).then(() => {
					...
					this.DOM.el.classList.remove('grid--animating3');
					resolve()
				});
			});
		}
		hide(filled = false, animation = true) {
			return new Promise((resolve, reject) => {
				...
				Promise.all(promises).then(() => {
					...
					this.DOM.el.classList.remove('grid--animating3');
					this.DOM.el.classList.remove('grid--current3');
					resolve();
				});
			});
		}
		animateText(el, dir) {
			return new Promise((resolve, reject) => {
				el.classList.add(`grid__item--animate${dir}3`);
				el.addEventListener('animationend', resolve);
			});
		}
		resetTextClasses(dir) {
			for (let i = 0, len = this.textElems.length; i < len; i++) {
				this.textElems[i].classList.remove(`grid__item--animate${dir}3`);
			};
		}
		hideItems() {
			for (let i = 0; i < this.itemsTotal; i++) {
	...
	class Slideshow {
		...
		initEvents() {
			Array.from(document.querySelectorAll('.grid__item--nav-next3')).forEach((ctrl) => ctrl.addEventListener('click', (ev) => this.navigate(ev, 'next')));
			Array.from(document.querySelectorAll('.grid__item--nav-prev3')).forEach((ctrl) => ctrl.addEventListener('click', (ev) => this.navigate(ev, 'prev')))
